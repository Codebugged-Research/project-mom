...
export default function Dashboard() {
  ...
  const channels = (data?.channels || CHANNELS).map(ch => ({
    ...ch,
    ...
    <div className="p-6 space-y-6 min-h-full">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-[#8B9A92] font-body mt-1">
            June 2026 · All Channels
          </p>
        </div>
        <span className="text-xs font-body uppercase tracking-widest text-white bg-white/10 border border-white/20 px-3 py-1.5 rounded-full">
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {KPI_CONFIG.map((cfg) => (
          <div
            ...
            className="bg-[#0A0A0A] border border-white/5 rounded-xl p-4 hover:border-white/20 hover:shadow-[0_4px_20px_rgba(255,255,255,0.02)] transition-all duration-300 group"
          >
            <div className="flex items-start justify-between mb-3">
              ...
              <div className={`flex items-center gap-0.5 text-xs font-mono ${cfg.isGood ? "text-white" : "text-neutral-400"}`}>
                {cfg.trendDown ? (
            ...
            <div className="text-2xl sm:text-3xl font-mono font-bold text-white mb-1 tracking-tight">
              {cfg.format(overview[cfg.key])}
            </div>
            <div className="text-[10px] uppercase tracking-[0.15em] text-neutral-500 font-body">
              {cfg.label}
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div
          ...
          className="lg:col-span-2 bg-[#0A0A0A] border border-white/5 rounded-xl p-5"
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-4 text-xs font-body">
              <span className="flex items-center gap-1.5 text-neutral-400">
                <span className="w-3 h-0.5 bg-neutral-600 inline-block rounded" />
                Spend
              </span>
              <span className="flex items-center gap-1.5 text-neutral-400">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={monthlyTrend} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="spend" name="Spend" stroke="#666666" strokeWidth={2} fill="url(#spendGrad)" dot={false} activeDot={{ r: 4, fill: "#666666" }} />
              <Area type="monotone" dataKey="revenue" name="Revenue" stroke="#FFFFFF" strokeWidth={2} fill="url(#revGrad)" dot={false} activeDot={{ r: 4, fill: "#FFFFFF" }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={180}>
            <LineChart data={monthlyTrend} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <Tooltip content={<CustomTooltip />} cursor={{ stroke: 'rgba(255, 255, 255, 0.15)', strokeWidth: 1 }} />
              <Line type="monotone" dataKey="cac" name="CAC" stroke="#FFFFFF" strokeWidth={2.5} dot={{ fill: "#FFFFFF", r: 3 }} activeDot={{ r: 5 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-3 text-xs font-body">
              <span className="flex items-center gap-1 text-neutral-400">
                <span className="w-2 h-2 bg-neutral-600 rounded-sm" />
                Spend
              </span>
              <span className="flex items-center gap-1 text-neutral-400">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={channels} margin={{ top: 0, right: 5, left: 0, bottom: 0 }} barCategoryGap="25%">
              ...
              <Bar dataKey="spend" name="Spend" fill="#666666" fillOpacity={0.6} radius={[2, 2, 0, 0]} />
              <Bar dataKey="revenue" name="Revenue" fill="#FFFFFF" fillOpacity={0.6} radius={[2, 2, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
